...
QString QCheckboxFilterModel::itemId(int row) const
{
  qDebug() << objectName() << "itemId";

  QString id;

  if(row >= 0)
  {
    QModelIndex src_ind = mapToSource(index(row, Checkboxed_Column));
    id = src_ind.data(Qt::UserRole).toString();

    qDebug() << row << "->" << src_ind.row();
  }
